...
class DbCartManager:
    ...
    def update_data(self, id_cart: int, state: str, id_user: int | None = None):
        with engine.connect() as conn:
            ...
            stmt = (
                update(cart_table).where(cart_table.c.id == id_cart).values(state=state)
            )
            result = conn.execute(stmt)
    ...
    def __verify_if_cart_is_active(self, id_user: int | None):
        if id_user is None:
            return False
        stmt = select(cart_table).where(
            and_(cart_table.c.id_user == id_user, cart_table.c.state == "active")
        )
        with engine.connect() as conn:
